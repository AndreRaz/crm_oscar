...
def catalog(client, db):
    make_user(db, "admin", "admin"); login(client, "admin"); part = client.post("/api/part-types", json={"part_number": "P1", "part_description": "Pump body"}).json()
    [client.post(f"/api/part-types/{part['id']}/characteristics", json={"control_plan": code, "measurement_method": "Digital caliper", "tol_type": "SYMMETRIC", "nominal": 10, "tol_plus": 1}) for code in ("A", "B")]; return part
def test_part_fields_are_required_and_editable(db, client):
    make_user(db, "admin", "admin"); login(client, "admin"); assert client.post("/api/part-types", json={"part_number": "P1"}).status_code == 422; assert client.post("/api/part-types", json={"part_number": "P1", "part_description": "Pump body"}).status_code == 201; assert client.patch("/api/part-types/1", json={"part_number": "VALVE-1", "part_description": "Valve stem"}).json()["part_number"] == "VALVE-1"
@pytest.mark.parametrize("payload", [{"control_plan": "N", "measurement_method": "Gauge", "tol_type": "SYMMETRIC", "nominal": 1, "tol_plus": -1}])
...
def test_selection_ownership_and_finite_measurements_are_enforced(db, client):
    ...
    started = client.post("/api/inspections", json={"part_type_id": part["id"], "characteristic_ids": [1]}).json()
    ...
    assert client.delete("/api/characteristics/1").status_code == 204; assert [c["control_plan"] for c in client.get(f"/api/part-types/{part['id']}/characteristics").json()] == ["B"]
    assert client.post("/api/inspections", json={"part_type_id": part["id"], "characteristic_ids": [1]}).status_code == 422; assert "A" in render_report_html(db, db.get(Inspection, started["id"]))
...
def test_non_finite_characteristic_numbers_are_rejected(db, client, field, value):
    part = catalog(client, db); payload = {"control_plan": "C", "measurement_method": "Digital caliper", "tol_type": "LIMITS", "nominal": 1, "tol_plus": 1, "min_limit": 0, "max_limit": 2}; payload[field] = float(value)
    responses = [client.post(f"/api/part-types/{part['id']}/characteristics", content=json.dumps(payload), headers={"content-type": "application/json"}), client.patch("/api/characteristics/1", content=json.dumps({field: float(value)}), headers={"content-type": "application/json"})]
    for response in responses:
        assert response.status_code == 422
        detail = response.json()["detail"]
        assert any(error["type"] == "finite_number" for error in detail)
        assert all(error.get("input") is None for error in detail)
