...
export default function App() {
  ...
  return <><header><div><strong>Control dimensional</strong><small>{user.username}</small></div><button onClick={async () => { await api.auth.logout(); setUser(null); }}>Cerrar sesión</button></header>
    ...
    <main>{page === "users" ? <Users /> : page === "catalog" ? <Catalog role={user.role} /> : page === "inspection" ? <Inspection /> : <section><h2>{tabs[user.role].find(([key]) => key === page)?.[1]}</h2><p>Disponible en la siguiente fase.</p></section>}</main>
  </>;
}
